...
interface UserRepository : CrudRepository<AkUserEntity, Int> {
    ...
    @Query(value = "SELECT * FROM (SELECT * FROM (SELECT *  FROM (SELECT (6371 *  acos(cos(radians(LAT)) * cos(radians(LAT_ORIGINAL)) *                         cos(radians(LON_ORIGINAL) - radians(LON)) +   sin(radians(LAT)) * sin(radians(LAT_ORIGINAL)))) * 1000 as distance,  A.* FROM (SELECT LAT AS LAT_ORIGINAL, LON AS LON_ORIGINAL FROM AK_POSITION  INNER JOIN AK_USER AU on AK_POSITION.ID = AU.POSITION_ID WHERE AU.ID = :userId) as LOLO  INNER JOIN AK_POSITION INNER JOIN AK_USER A on AK_POSITION.ID = A.POSITION_ID  WHERE A.ID != :userId) as `d*` WHERE distance < :distanceInMeters) C UNION ALL SELECT FLOOR(RAND() * (:distanceInMeters - 1) + 1) as distance, B.*FROM AK_USER B WHERE ID < 120) D GROUP BY ID ORDER BY ID LIMIT :limit OFFSET :offset", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM (SELECT * FROM (SELECT (6371 * acos(cos(radians(LAT)) * cos(radians(:latOriginal)) * cos(radians(:lonOriginal) - radians(LON)) +  sin(radians(LAT)) * sin(radians(:latOriginal)))) * 1000 as distance, A.* FROM AK_POSITION INNER JOIN AK_USER A on AK_POSITION.ID = A.POSITION_ID  WHERE A.ID != :userId) as `d*` WHERE distance < :distanceInMeters UNION ALL SELECT FLOOR(RAND() * (:distanceInMeters - 1) + 1) as distance, B.*FROM AK_USER B WHERE ID < 120 ) F GROUP BY ID ORDER BY ID LIMIT :limit OFFSET :offset", nativeQuery = true)
