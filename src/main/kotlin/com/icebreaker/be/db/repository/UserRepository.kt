...
interface UserRepository : CrudRepository<AkUserEntity, Int> {
    ...
    @Query(value = "SELECT * FROM (SELECT (6371 * acos( cos(radians(LAT)) * cos(radians(LAT_ORIGINAL)) * cos(radians(LON_ORIGINAL) - radians(LON)) + sin(radians(LAT)) * sin(radians(LAT_ORIGINAL)) )) * 1000 as distance, A.* FROM (SELECT LAT AS LAT_ORIGINAL, LON AS LON_ORIGINAL FROM AK_POSITION INNER JOIN AK_USER AU on AK_POSITION.ID = AU.POSITION_ID WHERE AU.ID = :userId) as LOLO INNER JOIN AK_POSITION INNER JOIN AK_USER A on AK_POSITION.ID = A.POSITION_ID WHERE A.ID != :userId) as `d*` WHERE distance < :distanceInMeters AND (:gender IS NULL OR GENDER =:gender) LIMIT :limit OFFSET :offset", nativeQuery = true)
    fun findUsersCloseToUser(@Param("userId") userId: Int, @Param("distanceInMeters") distanceInMeters: Int, @Param("gender") gender: Int?, @Param("limit") limit: Int, @Param("offset") offset: Int): List<Map<String, Any>>

    @Query(value = "SELECT * FROM (SELECT (6371 * acos( cos(radians(LAT)) * cos(radians( :latOriginal)) * cos(radians( :lonOriginal) - radians(LON)) + sin(radians(LAT)) * sin(radians(:latOriginal)) )) * 1000 as distance, A.* FROM AK_POSITION INNER JOIN AK_USER A on AK_POSITION.ID = A.POSITION_ID WHERE A.ID != :userId) as `d*` WHERE distance < :distanceInMeters AND (:gender IS NULL OR GENDER =:gender) LIMIT :limit OFFSET :offset", nativeQuery = true)
    fun findUsersCloseToUserPosition(@Param("userId") userId: Int, @Param("distanceInMeters") distanceInMeters: Int, @Param("latOriginal") latOriginal: Double, @Param("lonOriginal") lonOriginal: Double, @Param("gender") gender: Int?, @Param("limit") limit: Int, @Param("offset") offset: Int): List<Map<String, Any>>


    @Query(value = "SELECT ID, ANY_VALUE(distance) AS DISTANCE, ANY_VALUE(FIRST_NAME) AS FIRST_NAME, ANY_VALUE(LAST_NAME) AS LAST_NAME, ANY_VALUE(EMAIL) AS EMAIL, ANY_VALUE(PASSWORD_HASH) AS PASSWORD_HASH, ANY_VALUE(IMG_URL) AS IMG_URL, ANY_VALUE(BIRTHDAY) AS BIRTHDAY, ANY_VALUE(BIO) AS BIO, ANY_VALUE(GENDER) AS GENDER, ANY_VALUE(CREATED_AT) AS CREATED_AT, ANY_VALUE(UPDATED_AT) AS UPDATED_AT, ANY_VALUE(ACCOUNT_EXPIRED) AS ACCOUNT_EXPIRED, ANY_VALUE(ACCOUNT_LOCKED) AS ACCOUNT_LOCKED, ANY_VALUE(CREDENTIALS_EXPIRED) AS CREDENTIALS_EXPIRED, ANY_VALUE(ENABLED) AS ENABLED, ANY_VALUE(POSITION_ID) AS POSITION_ID, ANY_VALUE(PUSH_ID) AS PUSH_ID, ANY_VALUE(LAST_SEEN) AS LAST_SEEN, ANY_VALUE(CREDITS) AS CREDITS, ANY_VALUE(CREDITS_UPDATED_AT) AS CREDITS_UPDATED_AT, ANY_VALUE(ADMOB_COUNT) AS ADMOB_COUNT, ANY_VALUE(ADMOB_UPDATED_AT) AS ADMOB_UPDATED_AT, ANY_VALUE(INVITED_BY) AS INVITED_BY FROM (SELECT * FROM (SELECT *  FROM (SELECT (6371 *  acos(cos(radians(LAT)) * cos(radians(LAT_ORIGINAL)) * cos(radians(LON_ORIGINAL) - radians(LON)) +   sin(radians(LAT)) * sin(radians(LAT_ORIGINAL)))) * 1000 as distance,  A.* FROM (SELECT LAT AS LAT_ORIGINAL, LON AS LON_ORIGINAL FROM AK_POSITION  INNER JOIN AK_USER AU on AK_POSITION.ID = AU.POSITION_ID WHERE AU.ID = :userId) as LOLO  INNER JOIN AK_POSITION INNER JOIN AK_USER A on AK_POSITION.ID = A.POSITION_ID  WHERE A.ID != :userId) as `d*` WHERE distance < :distanceInMeters) C UNION ALL SELECT FLOOR(RAND() * (:distanceInMeters - 1) + 1) as distance, B.*FROM AK_USER B WHERE ID < 120 AND ID != :userId) D WHERE (:gender IS NULL OR GENDER =:gender) GROUP BY ID ORDER BY ID DESC LIMIT :limit OFFSET :offset", nativeQuery = true)
    fun findUsersCloseToUserWithFakeUsers(@Param("userId") userId: Int, @Param("distanceInMeters") distanceInMeters: Int, @Param("gender") gender: Int?, @Param("limit") limit: Int, @Param("offset") offset: Int): List<Map<String, Any>>

    @Query(value = "SELECT ID, ANY_VALUE(distance) AS DISTANCE, ANY_VALUE(FIRST_NAME) AS FIRST_NAME, ANY_VALUE(LAST_NAME) AS LAST_NAME, ANY_VALUE(EMAIL) AS EMAIL, ANY_VALUE(PASSWORD_HASH) AS PASSWORD_HASH, ANY_VALUE(IMG_URL) AS IMG_URL, ANY_VALUE(BIRTHDAY) AS BIRTHDAY, ANY_VALUE(BIO) AS BIO, ANY_VALUE(GENDER) AS GENDER, ANY_VALUE(CREATED_AT) AS CREATED_AT, ANY_VALUE(UPDATED_AT) AS UPDATED_AT, ANY_VALUE(ACCOUNT_EXPIRED) AS ACCOUNT_EXPIRED, ANY_VALUE(ACCOUNT_LOCKED) AS ACCOUNT_LOCKED, ANY_VALUE(CREDENTIALS_EXPIRED) AS CREDENTIALS_EXPIRED, ANY_VALUE(ENABLED) AS ENABLED, ANY_VALUE(POSITION_ID) AS POSITION_ID, ANY_VALUE(PUSH_ID) AS PUSH_ID, ANY_VALUE(LAST_SEEN) AS LAST_SEEN, ANY_VALUE(CREDITS) AS CREDITS, ANY_VALUE(CREDITS_UPDATED_AT) AS CREDITS_UPDATED_AT, ANY_VALUE(ADMOB_COUNT) AS ADMOB_COUNT, ANY_VALUE(ADMOB_UPDATED_AT) AS ADMOB_UPDATED_AT, ANY_VALUE(INVITED_BY) AS INVITED_BY FROM (SELECT * FROM (SELECT (6371 * acos(cos(radians(LAT)) * cos(radians(:latOriginal)) * cos(radians(:lonOriginal) - radians(LON)) +  sin(radians(LAT)) * sin(radians(:latOriginal)))) * 1000 as distance, A.* FROM AK_POSITION INNER JOIN AK_USER A on AK_POSITION.ID = A.POSITION_ID  WHERE A.ID != :userId) as `d*` WHERE distance < :distanceInMeters UNION ALL SELECT FLOOR(RAND() * (:distanceInMeters - 1) + 1) as distance, B.*FROM AK_USER B WHERE ID < 120 AND ID != :userId ) F WHERE (:gender IS NULL OR GENDER =:gender) GROUP BY ID ORDER BY ID DESC LIMIT :limit OFFSET :offset", nativeQuery = true)
    fun findUsersCloseToUserPositionWithFakeUsers(@Param("userId") userId: Int, @Param("distanceInMeters") distanceInMeters: Int, @Param("latOriginal") latOriginal: Double, @Param("lonOriginal") lonOriginal: Double, @Param("gender") gender: Int?, @Param("limit") limit: Int, @Param("offset") offset: Int): List<Map<String, Any>>
