...
interface UserRepository : CrudRepository<AkUserEntity, Int> {
    ...
    @Query(value = "SELECT ID, ANY_VALUE(distance), ANY_VALUE(FIRST_NAME), ANY_VALUE(LAST_NAME), ANY_VALUE(EMAIL), ANY_VALUE(PASSWORD_HASH), ANY_VALUE(IMG_URL), ANY_VALUE(BIRTHDAY), ANY_VALUE(BIO), ANY_VALUE(GENDER), ANY_VALUE(CREATED_AT), ANY_VALUE(UPDATED_AT), ANY_VALUE(ACCOUNT_EXPIRED), ANY_VALUE(ACCOUNT_LOCKED), ANY_VALUE(CREDENTIALS_EXPIRED), ANY_VALUE(ENABLED), ANY_VALUE(POSITION_ID), ANY_VALUE(PUSH_ID), ANY_VALUE(LAST_SEEN), ANY_VALUE(CREDITS), ANY_VALUE(CREDITS_UPDATED_AT), ANY_VALUE(ADMOB_COUNT), ANY_VALUE(ADMOB_UPDATED_AT), ANY_VALUE(INVITED_BY) FROM (SELECT * FROM (SELECT *  FROM (SELECT (6371 *  acos(cos(radians(LAT)) * cos(radians(LAT_ORIGINAL)) * cos(radians(LON_ORIGINAL) - radians(LON)) +   sin(radians(LAT)) * sin(radians(LAT_ORIGINAL)))) * 1000 as distance,  A.* FROM (SELECT LAT AS LAT_ORIGINAL, LON AS LON_ORIGINAL FROM AK_POSITION  INNER JOIN AK_USER AU on AK_POSITION.ID = AU.POSITION_ID WHERE AU.ID = :userId) as LOLO  INNER JOIN AK_POSITION INNER JOIN AK_USER A on AK_POSITION.ID = A.POSITION_ID  WHERE A.ID != :userId) as `d*` WHERE distance < :distanceInMeters) C UNION ALL SELECT FLOOR(RAND() * (:distanceInMeters - 1) + 1) as distance, B.*FROM AK_USER B WHERE ID < 120 AND ID != :userId) D GROUP BY ID ORDER BY ANY_VALUE(distance) LIMIT :limit OFFSET :offset", nativeQuery = true)
    ...
    @Query(value = "SELECT ID, ANY_VALUE(distance), ANY_VALUE(FIRST_NAME), ANY_VALUE(LAST_NAME), ANY_VALUE(EMAIL), ANY_VALUE(PASSWORD_HASH), ANY_VALUE(IMG_URL), ANY_VALUE(BIRTHDAY), ANY_VALUE(BIO), ANY_VALUE(GENDER), ANY_VALUE(CREATED_AT), ANY_VALUE(UPDATED_AT), ANY_VALUE(ACCOUNT_EXPIRED), ANY_VALUE(ACCOUNT_LOCKED), ANY_VALUE(CREDENTIALS_EXPIRED), ANY_VALUE(ENABLED), ANY_VALUE(POSITION_ID), ANY_VALUE(PUSH_ID), ANY_VALUE(LAST_SEEN), ANY_VALUE(CREDITS), ANY_VALUE(CREDITS_UPDATED_AT), ANY_VALUE(ADMOB_COUNT), ANY_VALUE(ADMOB_UPDATED_AT), ANY_VALUE(INVITED_BY) FROM (SELECT * FROM (SELECT (6371 * acos(cos(radians(LAT)) * cos(radians(:latOriginal)) * cos(radians(:lonOriginal) - radians(LON)) +  sin(radians(LAT)) * sin(radians(:latOriginal)))) * 1000 as distance, A.* FROM AK_POSITION INNER JOIN AK_USER A on AK_POSITION.ID = A.POSITION_ID  WHERE A.ID != :userId) as `d*` WHERE distance < :distanceInMeters UNION ALL SELECT FLOOR(RAND() * (:distanceInMeters - 1) + 1) as distance, B.*FROM AK_USER B WHERE ID < 120 AND ID != :userId ) F GROUP BY ID ORDER BY ANY_VALUE(distance) LIMIT :limit OFFSET :offset", nativeQuery = true)
